...
public class MockTestBase {
    ...
    protected void setBasicMockProperties(boolean withConnectionService) throws InitializationException {
        if(testRunner != null) {
            testRunner.setProperty(AbstractCosmosDocumentProcessor.DATABASE_NAME, MOCK_DB_NAME);
            testRunner.setProperty(AbstractCosmosDocumentProcessor.CONTAINER_ID,MOCK_CONTAINER_ID);
            testRunner.setProperty(AbstractCosmosDocumentProcessor.PARTITION_KEY,MOCK_PARTITION_FIELD_NAME);
            if(withConnectionService) {
                // setup connnection controller service
                AzureCosmosDocumentConnectionControllerService service = new MockConnectionService();
    ...
    private class MockConnectionService extends AzureCosmosDocumentConnectionControllerService {
        @Override
